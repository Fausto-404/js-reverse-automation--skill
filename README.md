...
相对 v2.1，本版本从“扩展观测与结果验证”进一步升级为“可持续对抗、可证明成功、可直接交付”的完整闭环。能力增强面向通用浏览器逆向流程，不针对单一靶题写死。

- **运行时对抗能力升级**：统一观测反调试、完整性校验、环境属性、动态代码、多 Realm 与加载器行为；通过持久 Hook 和统一编排器支持探针叠加、外部替换后的重建以及安全卸载，提升复杂混淆环境下的持续取证能力。
- **JSVMP 与重度混淆分析升级**：增加保守的源码级属性 tap，关联插桩位置、运行时热点和调用结果；配合基线/干预差分，区分有效证据、错误增长和干预副作用，减少只看静态代码的误判。
- **真实浏览器会话升级**：绑定 tab 与 document 身份，识别导航、断线、超时和暂停后的失效引用；观测控制台清理、历史导航、存储访问和页面关闭等控制流信号，并支持可回滚的受控干预。
- **成功判定升级**：新增 action 级证据闭环，必须同时验证明文、最终请求体、最终路由、HTTP 状态和业务响应，明确区分服务可连接、动作已触发、请求链成功和业务验证通过，避免把端口监听或 HTTP 200 当成逆向成功。
- **传输与证据可靠性升级**：最终请求捕获统一覆盖 fetch 与 XMLHttpRequest；原始对象采用有界、循环引用安全的快照；兼容历史 JSRPC 外层字符串数据，避免 CryptoJS、JSEncrypt 等对象或响应封装导致证据丢失。
...
最终交付格式保持稳定：继续输出明文、密文/签名、JSRPC、Flask、Burp autoDecoder 配置和验证命令；对抗观测作为可靠性证据补充，不改变使用方式。
