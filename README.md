...
相对 v2.1，本版本从“扩展观测与结果验证”进一步升级为“可持续对抗、可证明成功、可直接交付”的完整闭环，面向通用浏览器逆向流程提供统一能力支撑。
...
最终交付格式保持稳定：继续输出明文、密文/签名、JSRPC、Flask、Burp autoDecoder 配置和验证命令；对抗观测同步纳入可靠性证据，保持原有使用方式。
